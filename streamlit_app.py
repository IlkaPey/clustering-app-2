...
import random 
import qrcode # <--- WIEDER HINZUGEFÜGT
from io import BytesIO # <--- WIEDER HINZUGEFÜGT
    ...
    with st.form("survey_form", clear_on_submit=True):
        ...
        if submitted:
            if not user_name.strip():
                ...
                conn.commit()
                conn.close()
                st.success(f"Danke {user_name}! Deine Daten wurden erfolgreich übertragen.")
                st.balloons()
...
if app_role == "presenter" and view == "📺 Präsentator: Live-Schritt-Demo":
    ...
    with col_control:
        ...
        base_url = "http://placeholder.url" # Fallback für den Fall, dass es schief geht
        ...
        # URL-Parameter für Teilnehmeransicht bereinigen
        ...
        st.code(participant_url, language="text") # Zeigt den Link auch als reinen Text an
